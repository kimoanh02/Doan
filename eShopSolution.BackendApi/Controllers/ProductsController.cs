...
namespace eShopSolution.BackendApi.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        [HttpGet("paging/{productId}/{languageId}")]
        [AllowAnonymous]
        public async Task<IActionResult> GetById1(int productId, string languageId)
        {
            var product = await _productService.GetById1(productId, languageId);
            if (product == null)
                return BadRequest("Cannot find product");
            return Ok(product);
        }
